...
def save_to_csv(results, token_name):
    filename = f"{token_name}_positions.csv"
    with open(filename, mode='w', newline='') as file:
        writer = csv.DictWriter(file, fieldnames=["Position", "Percent", "Amount", "Cost"])
        writer.writeheader()
        for row in results:
            writer.writerow(row)
